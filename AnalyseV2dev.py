...
def bol_contains(dataframe, key_wrd):
    contains = False
    for column in dataframe.columns:
        if key_wrd in column.lower():
            contains = True
        else:
            continue
...
def program():
    ...
    while run:
        ...
        print("7) Open several files and save them as excel sheets.")
        ...
        if user_choice == '1':
            ...
            while load_run:
                ...
                try:
                    ...
                    if bol_contains(df, "pro"):
                        num_fish_count = count_fish(df, "X", 1)
                        df = clean_calc(df, num_fish_count)
                        print("Tables were cleaned, and filter was added")
                        print("")
                    else:
                ...
                print("If you do, type 1 and press enter")
                print("")
                ...
                if load_choice is not "":
                    try:
                        load_choice = filedialog.askopenfilename()
    ...
    # --------------- PLOTTING MENU ---------------------
        ...
        # -------------- SAVE FUNCTION ---------------
            ...
            print("3) to save all well dataframes in excel format.")
        ...
        # ----------- DISTANCE CALCULATIONS MENU -------------------
            ...
            try:
                new_name = input("New file name (must en with .xlsx): ")
                writer = pd.ExcelWriter(new_name, engine="xlsxwriter")
                num_files = int(input("How many files do you want to use/open? "))

                for i in range(0, num_files):
                    name_well = "df_well_" + str(i)
                    load_run = True
                    while load_run:
                        ...
                        try:
                            ...
                            name_well = pd.read_csv(file_name, sep="\t", header=0, )
                            if bol_contains(name_well, "pro"):
                                num_fish_count = count_fish(name_well, "X", 1)
                                name_well = clean_calc(name_well, num_fish_count)
                                print("Tables were cleaned, and filter was added")
                                print("")
                            ...
                            print("File loaded to well DataFrame df_well_" + str(i))
                            print(" ")
                            name_sheet = input("Please name the sheet: ")
                            print("Writing file to excel sheet, this may take a few seconds.")
                            print("Loading..")
                            name_well = name_well.round(2)
                            name_well.to_excel(writer, name_sheet)
                            print("Done writing file df_well_" + str(i) + " to the sheet.")
                            ...
                            print("")
                            load_run = False
                print("Saving all sheets to excel file")
                print("Working..")
                writer.save()
                ...
                print("Something went wrong when trying to use option 7.")
